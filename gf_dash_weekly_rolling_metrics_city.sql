/*
    Name: slide.gf_dash_weekly_rolling_metrics_city
...
(
  ...
  AND mau_info.city = first_bk_country.city
...
AND base.city = pax_conversion.city

LEFT JOIN

(SELECT
churn_reactivated_user.country,
churn_reactivated_user.city,
...
,CASE WHEN date(min_order_time)>=date(week_of) - INTERVAL '28' DAY AND date(min_order_time) < date(week_of) THEN 1 ELSE 0 END AS active_first_4_weeks
  ...
  WHERE latest_rank=1) churn_reactivated_user
