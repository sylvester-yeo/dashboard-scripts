/*
    Name: slide.gf_dash_weekly_rolling_metrics_region
...
(
  ...
  (
	  ...
	  ON 
	  mau_info.passenger_id = first_bk_country.passenger_id
...
(
	...
	,CASE WHEN date(min_order_time)>=date(week_of) - INTERVAL '28' DAY AND date(min_order_time) < date(week_of) THEN 1 ELSE 0 END AS active_first_4_weeks
	  ...
	  (
		  ...
		  passenger_id,
		  evaluation_week.week_of,
		  max(ride_hist.date_local) AS max_order_time,
		  min(ride_hist.date_local) AS min_order_time
...
,CASE WHEN date(min_bid_time)>=date(week_of) - INTERVAL '28' DAY AND date(min_bid_time) < date(week_of) THEN 1 ELSE 0 END AS active_first_4_weeks
  ...
  driver_id,
  evaluation_week.week_of,
  max(date_local) AS max_bid_time,
  min(date_local) AS min_bid_time
...
GROUP BY 1 ) churn_reactivation_driver
