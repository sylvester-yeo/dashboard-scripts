  ...
  from
    ...
    select * from (
        ...
        where date(concat(year,'-',month,'-',day)) < date('2019-06-20')
	        and date(concat(year,'-',month,'-',day)) >= date('2019-01-01')
        )
    where row_num = 1
    )
)
,mex as (
    ...
    select * from (
        ...
        from snapshots.grab_mall_grab_mall_seller
        where
            date(concat(year,'-',month,'-',day)) >= date('2019-01-01')
...
,orders as (
  ...
  WHERE date(a.partition_date_local) >= date('2019-01-01')
             and date(a.partition_date_local) < date('2020-01-01') 
...
where date(date_local) < date('2020-01-01')
